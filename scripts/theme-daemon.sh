...
# đổi wallpaper Sáng/Tối, xem scripts/make-theme-daemon.sh) TỪ SOURCE đã
...
#
# Nguồn được sinh sẵn ở HOST bằng `scripts/make-theme-daemon.sh` (chỉ ghi
# file .cpp/CMakeLists/.service/.preset, không cần libxfconf trên runner)
# rồi workflow copy nguyên cây app-for-hyggshi/hyggshi-theme-daemon/ vào
...
if [ ! -f "$SRC_DIR/CMakeLists.txt" ]; then
  ...
  echo "đã được sinh bằng scripts/make-theme-daemon.sh và copy vào chroot trước." >&2
  exit 1
fi
