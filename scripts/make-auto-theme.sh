...
# hyggshi-theme-daemon (C++/libxfconf, xem scripts/make-theme-daemon.sh).
...
#
# Cách dùng:
#   ./scripts/make-auto-theme.sh     # chỉ sinh source vào app-for-hyggshi/hyggshi-theme/
#
# Việc CÀI THẬT vào hệ thống (copy file + systemctl enable) do
# app-for-hyggshi/auto-theme.sh đảm nhiệm, chạy trong chroot lúc build ISO
...
APP_DIR="$REPO_ROOT/app-for-hyggshi/hyggshi-theme"
...
`app-for-hyggshi/hyggshi-theme-daemon/`). Thay vì 1 daemon chạy nền liên
...
`/usr/local/bin/hyggshi-set-wallpaper.sh` (cài bởi `scripts/branding.sh`)
